    ...
    for i in range(len(articles)):
        ...
        if j < 5:  # 날짜
            if articles_detail[i].text[3:].split(' ')[1][:6] == 'minute':
                article_date.append(
                    (dt - datetime.timedelta(minutes=int(articles_detail[i].text[3:].split(' ')[0]))).strftime(
                        "%b %d, %Y"))
            elif articles_detail[i].text[3:].split(' ')[1][:4] == 'hour':
                article_date.append(
                    (dt - datetime.timedelta(hours=int(articles_detail[i].text[3:].split(' ')[0]))).strftime(
                        "%b %d, %Y"))
...
df = pd.DataFrame({"Label": article_label, "Title": article_title, "Link": article_link,
